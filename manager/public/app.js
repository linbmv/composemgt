      ...
      <div class="card-actions">
        ...
        <div class="action-right">
          ...
          ${service.deploySource === 'build'
            ? `<button class="btn btn-primary btn-xs" onclick="triggerAction('${service.name}', 'build-update')" title="在构建目录执行 git pull 拉取最新代码 → 重新构建镜像 → 重启容器">
                🔄 更新
              </button>`
            : `<button class="btn btn-primary btn-xs" onclick="triggerAction('${service.name}', 'pull')" title="从远程镜像仓库拉取最新镜像版本 → 重启容器">
                🔄 更新
              </button>`
          }
